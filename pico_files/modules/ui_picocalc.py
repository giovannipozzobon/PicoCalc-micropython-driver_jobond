...
# Inizializza display e tastiera
display = PicoDisplay(320, 240)
keyboard = PicoKeyboard()
...
class Label(Widget):
    ...
    def draw(self, tl, force_draw = False ):
        ...
        #  Draw background
        sz = self.size()
        display.fill_rect( tl[0], tl[1],
                          sz[0], sz[1],
                          self.background_color )

        #  Draw the text
        display.text( self.title,
                          tl[0] + 10,
...
class Text_Input( Widget ):
    ...
    def draw( self, tl, force_draw = False ):
        ...
        display.fill_rect( start_input_x, 
                          tl[1],
                          self.input_width - 1, sz[1]-2,
                          self.label_bg_color )

        #  Draw the text
        display.text( self.input_text,
...
class Button( Widget ):
    ...
    def size( self ):
        return np.array( [300, 30], dtype = np.float )
    ...
    def draw( self, tl, force_draw = False ):
        ...
        display.fill_rect( tl[0] + self.padding, tl[1] + self.padding,
                          sz[0] - self.padding * 2,
                          sz[1] - self.padding * 2,
                          self.button_color )
        ...
        display.text( self.title,
                          tl[0] + self.padding + 5,
                          tl[1] + sz[1] - self.padding - 5, colors.GS4.CYAN )
...
class Check_Box( Widget ):
    ...
    def draw( self, tl, force_draw = False ):
        ...
        display.fill_rect( offset[0],
                          offset[1],
                          self.box_height,
                          self.box_height )
        
        if self.checked:
            display.draw_line( offset[0],
                              offset[1],
                              offset[0] + self.box_height,
                              offset[1] + self.box_height )
            display.draw_line( offset[0] + self.box_height,
                              offset[1],
                              offset[0],
                              offset[1] + self.box_height )
